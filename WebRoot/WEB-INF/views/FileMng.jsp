...
<body>
	<div class="smart-green">
		<form name="myform" action="uploadServlet" method="post"
			...
			<h1>
				素材管理 <br /> <span>企业公众号素材管理.</span>
			</h1>
			<!-- <label> 
				<span>发 送 人 :</span> 
				<input id="fromUser" type="text"name="fromUser"/>
			</label>  -->
			<label> <span>接 收 人 [英文逗号或竖线分隔多个]:</span> <input id="toUser"
				type="text" name="toUser" />
			</label> <label> <span>触发时间 [若不填则默认立即发送]:</span> <input type="text"
				class="sang_Calender" id="sendTime" name="sendTime" />
			</label> <label> <span>消息类型 :</span> <select name="msgType"
				id="msgType"
				onchange="typeChange(this.options[this.options.selectedIndex].value)">
					<option value="">请选择...</option>
					<option value="image">图片[微信仅支持：jpg、png]</option>
					<option value="mpnews">图文[可用html标签增加样式]</option>
			...
			<div class="media">
				<label> <span>请选择文件 :</span> <br />
				<br /> <input type="file" name="myfile" id="upload-file"
					style="border: 3px;">
				</label>
			</div>
			<div class="submit">
